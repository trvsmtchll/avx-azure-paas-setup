...
# This script handles:
# 1. Creating a custom IAM role with Aviatrix required permissions
...
# Usage function
usage() {
    echo -e "${BLUE}Aviatrix Azure IAM Setup Script${NC}"
    echo ""
    echo "Usage: $0 [options]"
    echo ""
    echo "Options:"
    echo "  -n, --name NAME       App registration name (required)"
    echo "  -r, --role NAME       Custom role name (default: Aviatrix-PaaS-Role)"
    echo "  -c, --create-app      Create new app registration (default: use existing)"
    echo "  -h, --help            Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 --name Aviatrix-PaaS-App --role Aviatrix-PaaS-Role --create-app"
    echo ""
    exit 1
}
...
echo -e "\n${BLUE}Step 1: Creating custom role '${ROLE_NAME}'...${NC}"
ROLE_DESCRIPTION="Custom role for Aviatrix PaaS"

# Create role definition JSON
ROLE_DEF_FILE="/tmp/aviatrix-role-definition.json"
...
if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Failed to create custom role.${NC}"
    exit 1
fi
...
echo -e "\n${BLUE}Step 4: Assigning custom role to service principal...${NC}"
...
echo -e "${BLUE}Custom Role:${NC} $ROLE_NAME"
echo ""
echo -e "${GREEN}These values can be used to configure Aviatrix PaaS access to Azure.${NC}"